...
import {
  BrandProfileData,
  BrandProfileService,
} from '@/src/api/BrandProfileService';
import {
  ContentDraft,
...
const JaneAvatar = ({
  ...
  >
    <span style={{ color: '#fff', fontWeight: 800, fontSize: size * 0.38 }}>
      J
    </span>
    {pulse && (
      <div
...
const UserAvatar = ({
  ...
  >
    <span style={{ color: '#E91E63', fontWeight: 700, fontSize: size * 0.28 }}>
      {initials}
    </span>
  </div>
);
  ...
  return (
    <div
      style={{
        ...
        height: s,
        borderRadius: 2,
        background: c[p?.toLowerCase()] ?? '#999',
        flexShrink: 0,
      }}
    />
  );
...
const Bd = ({
  ...
  const m: Record<BadgeV, { bg: string; c: string; b: string }> = {
    ...
    success: {
      bg: 'rgba(76,175,80,.08)',
      c: '#2e7d32',
      b: 'rgba(76,175,80,.15)',
    },
    danger: {
      bg: 'rgba(155,44,61,.08)',
      c: '#9b2c3d',
      b: 'rgba(155,44,61,.15)',
    },
    warning: {
...
const platformGradient = (platform: string) => {
  ...
  return (
    m[platform?.toLowerCase()] ?? 'linear-gradient(135deg,#37474F,#607D8B)'
  );
};
...
const SchedulePage = ({ onJane }: { onJane: () => void }) => {
  ...
  const filtered = posts.filter((p) => {
    ...
    if (platformFilter !== 'all' && p.platform.toLowerCase() !== platformFilter)
      return false;
  ...
  const approvePost = async (post: PostItem) => {
    ...
    await SocialMediaAgentService.approveContent({
      draft_ids: [draftId],
      schedule_option: 'save_draft',
    });
    setPosts((ps) =>
      ps.map((x) => (x.id === post.id ? { ...x, status: 'scheduled' } : x)),
    );
  };

  const rejectPost = async (post: PostItem) => {
    const draftId = post.raw?.draft_id ?? post.raw?.id ?? post.id;
    await SocialMediaAgentService.denyContent({
      draft_ids: [draftId],
  ...
  const deletePost = async (post: PostItem) => {
    const draftId = post.raw?.draft_id ?? post.raw?.id ?? post.id;
    try {
      await SocialMediaAgentService.deleteDraft(draftId);
    } catch {
      /* ignore */
    }
    setPosts((ps) => ps.filter((x) => x.id !== post.id));
  ...
  const bulkApprove = async () => {
    const toApprove = posts.filter(
      (x) => selected.includes(x.id) && x.status === 'queue',
    );
  ...
  const handleAiCreate = async () => {
    ...
    try {
      ...
      const drafts: ContentDraft[] =
        (res as unknown as { responseData: { drafts: ContentDraft[] } })
          .responseData?.drafts ?? [];
      if (res.status && drafts.length > 0) {
        setAiResult(drafts);
        setPosts((ps) => [
          ...ps,
          ...drafts.map((d) => draftToItem(d, mondayMs)),
        ]);
      }
    } finally {
      setAiThinking(false);
  ...
  const calPosts = posts.filter(
    (p) =>
      (p.status === 'queue' || p.status === 'scheduled') && p.dayIndex != null,
  );
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12.5, color: '#999', margin: '2px 0 0' }}>
            Plan, review, and track all your content
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              fontFamily: 'var(--wf)',
              boxShadow: '0 2px 8px rgba(194,24,91,.25)',
            }}
          >
        ...
        >
          {WEEK_DAYS.map((day, i) => {
            ...
            return (
              <button
                key={day}
                onClick={() => setCalendarDay(isSel ? null : i)}
                style={{
                  padding: '8px 6px',
                  borderRadius: 10,
                  border: isSel
              ...
              >
                ...
                >
                  {dayPosts.slice(0, 3).map((p) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: '2px 5px',
                        borderRadius: 4,
                        background: `${statusColors[p.status]}08`,
                        borderLeft: `2px solid ${statusColors[p.status]}`,
                      }}
                    >
                  ...
                  {dayPosts.length === 0 && (
                    ...
                    >
                      <span style={{ fontSize: 9, color: '#ddd' }}>Empty</span>
                    </div>
                  )}
                </div>
                {dayPosts.length > 0 && (
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                border:
                  filter === f.id ? '1.5px solid #222' : '1.5px solid #e5e3df',
                background: filter === f.id ? '#222' : '#fff',
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  padding: '5px 12px',
                  borderRadius: 6,
                  border: '1.5px solid rgba(155,44,61,.2)',
                  background: '#fff',
                  color: '#9b2c3d',
                  fontSize: 11.5,
                  fontWeight: 600,
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 24px 20px' }}>
        {filter === 'queue' && queueCount > 0 && (
          ...
          >
            ...
            <span style={{ fontSize: 12.5, color: '#555' }}>
              <strong style={{ color: '#C2185B' }}>URI Agent</strong> drafted{' '}
              {queueCount} post{queueCount > 1 ? 's' : ''} waiting for your
              approval.
            </span>
          </div>
        )}
        {filter === 'draft' && (
          ...
          >
            <I n="edit" s={15} c="#999" />
            <span style={{ fontSize: 12.5, color: '#555' }}>
              Works in progress. Generate content from Workspace or create
              manually.
            </span>
          </div>
        )}
        ...
        {filtered.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '48px 20px' }}>
            ...
            <p style={{ fontSize: 12.5, color: '#999', margin: '0 0 12px' }}>
          ...
          {filtered.map((post) => {
            ...
            return (
              <div
                ...
                style={{
                  borderRadius: 11,
                  border: isSel ? '2px solid #C2185B' : '1px solid #edecea',
                  background: '#fff',
                  overflow: 'hidden',
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      height: 20,
                      borderRadius: 5,
                      border: isSel ? 'none' : '2px solid #ddd',
                      background: isSel ? '#C2185B' : '#fff',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      cursor: 'pointer',
                  ...
                  <div
                    style={{
                      ...
                      height: 44,
                      borderRadius: 9,
                      background: platformGradient(post.platform),
                      flexShrink: 0,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                    ...
                    {post.time && (
                      ...
                      >
                        <I
                          n="clock"
                          s={12}
                          c={post.status === 'scheduled' ? '#4caf50' : '#bbb'}
                        />
                        <span
                          style={{
                            fontSize: 11.5,
                            color:
                              post.status === 'scheduled' ? '#2e7d32' : '#999',
                            fontWeight: post.status === 'scheduled' ? 600 : 400,
                          }}
                        >
                          {post.time}
                        </span>
                  ...
                  <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                      ...
                      <>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            height: 32,
                            borderRadius: 7,
                            border: '1.5px solid rgba(155,44,61,.15)',
                            background: '#fff',
                            cursor: 'pointer',
                            display: 'flex',
                            alignItems: 'center',
                    ...
                    <button
                      ...
                      style={{
                        ...
                        height: 32,
                        borderRadius: 7,
                        border: '1.5px solid rgba(155,44,61,.1)',
                        background: '#fff',
                        cursor: 'pointer',
                        display: 'flex',
                        alignItems: 'center',
                ...
                {post.status === 'published' && post.eng && (
                  ...
                  >
                    {(
                      ...
                      ] as { l: string; v: string | number | undefined }[]
                    ).map((m, idx, arr) => (
                      <div
                        key={m.l}
                        style={{
                          ...
                          padding: '8px 12px',
                          borderRight:
                            idx < arr.length - 1 ? '1px solid #f0eeea' : 'none',
                          textAlign: 'center',
                        }}
                      >
                        <div
                        ...
                        <div style={{ fontSize: 9.5, color: '#999' }}>
                          {m.l}
                        </div>
                      </div>
                    ))}
      ...
      >
        <JaneAvatar size={26} />
        <input
          placeholder="Ask URI Agent about your schedule..."
          style={{
            flex: 1,
            padding: '9px 13px',
            borderRadius: 9,
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    padding: 9,
                    borderRadius: 8,
                    background: createMode === 'ai' ? '#fff' : 'transparent',
                    border: 'none',
                    fontFamily: 'var(--wf)',
                    fontSize: 13,
                    fontWeight: createMode === 'ai' ? 700 : 500,
                    color: createMode === 'ai' ? '#C2185B' : '#999',
                    cursor: 'pointer',
                    boxShadow:
                      createMode === 'ai'
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 8,
                    background:
                      createMode === 'manual' ? '#fff' : 'transparent',
                    border: 'none',
                    fontFamily: 'var(--wf)',
                    fontSize: 13,
                    fontWeight: createMode === 'manual' ? 700 : 500,
                    color: createMode === 'manual' ? '#111' : '#999',
                    cursor: 'pointer',
                    boxShadow:
                      createMode === 'manual'
                        ? '0 1px 4px rgba(0,0,0,.06)'
              ...
              {createMode === 'ai' ? (
                <div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11.5, color: '#999' }}>
                        Describe what you want and I'll draft it in your brand
                        voice.
                      </div>
                    </div>
                  </div>
                  <textarea
                    ...
                    placeholder="e.g. Create a post announcing our new product launch. Make it exciting with a sense of urgency..."
                    rows={4}
                    style={
                      {
                        width: '100%',
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: 'none',
                      background:
                        aiPrompt.trim() && !aiThinking ? '#C2185B' : '#eee',
                      color: aiPrompt.trim() && !aiThinking ? '#fff' : '#ccc',
                      fontWeight: 700,
                      fontSize: 13.5,
                      cursor:
                        aiPrompt.trim() && !aiThinking ? 'pointer' : 'default',
                      fontFamily: 'var(--wf)',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                  ...
                  >
                    {aiThinking ? (
                      ...
                      <>
                        <I n="sparkle" s={15} c="#fff" /> Generate Draft
                      </>
                    )}
                  </button>
                ...
                <div>
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                      {['Instagram', 'LinkedIn', 'X', 'Facebook'].map(
                        (plat) => (
                          <button
                            key={plat}
                            ...
                            style={{
                              ...
                              border:
                                manualPlatform === plat
                                  ...
                                  : '1.5px solid #e5e3df',
                              background:
                                manualPlatform === plat ? '#222' : '#fff',
                              color: manualPlatform === plat ? '#fff' : '#666',
                              fontSize: 12,
                              fontWeight: 600,
                              cursor: 'pointer',
                              fontFamily: 'var(--wf)',
                          ...
                          style={{
                            ...
                            background:
                              manualPillar === p
                                ? 'rgba(194,24,91,.06)'
                                : '#fff',
                            color: manualPillar === p ? '#C2185B' : '#666',
                            fontSize: 11.5,
                            fontWeight: 600,
                            cursor: 'pointer',
                            fontFamily: 'var(--wf)',
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 9,
                      border: 'none',
                      background: manualCaption.trim() ? '#C2185B' : '#eee',
                      color: manualCaption.trim() ? '#fff' : '#ccc',
                      fontWeight: 700,
                      fontSize: 13.5,
                      cursor: manualCaption.trim() ? 'pointer' : 'default',
                      fontFamily: 'var(--wf)',
                    }}
                  >
                  ...
                  >
                    ...
                    <span style={{ fontSize: 12, color: '#666' }}>
                      Want the agent to improve this? Switch to{' '}
                      <button
                        onClick={() => setCreateMode('ai')}
                        style={{
                          background: 'none',
...
const ConnectionsPage = ({ onJane }: { onJane: () => void }) => {
  ...
  const saveWaCache = (phone: string) => {
    try {
      localStorage.setItem(
        WA_CACHE_KEY,
        JSON.stringify({ linked: true, phone }),
      );
    } catch {
      /* noop */
    }
  ...
  const loadStatuses = async () => {
    ...
    // Use API if it responds, otherwise fall back to localStorage cache
  ...
  const handleWhatsappSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        // 409 — number is already connected, persist and show as connected
        saveWaCache(waPhone.trim());
        setWaExpanded(false);
        setWaPhone('');
        setStatuses((prev) => ({
          ...prev,
          whatsapp: { linked: true, phone: waPhone.trim() },
        }));
      } else {
        setWaError(
          detail ||
              ...
              <div key={p.id}>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {linked ? (
                      <div
                        style={{ fontSize: 11.5, color: '#555', marginTop: 1 }}
                      >
                        {s?.account_name ||
                          s?.username ||
                          s?.phone ||
                      ...
                      <div
                        style={{ fontSize: 11.5, color: '#bbb', marginTop: 1 }}
                      >
                        Not connected
                      </div>
                    )}
                ...
                {p.id === 'whatsapp' && waExpanded && !linked && (
                  ...
                  >
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === 'Enter' && handleWhatsappSubmit()
                      }
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                        ...
                        style={{
                          ...
                          fontFamily: 'var(--wf)',
                          opacity:
                            !waPhone.trim() || connecting === 'whatsapp'
                              ? 0.5
                              : 1,
                        }}
                      >
...
const SubPage = ({
  ...
  <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
    <div
      style={{ padding: '18px 24px 14px', borderBottom: '1px solid #edecea' }}
    >
      <h2
        style={{
          fontSize: 17,
      ...
      [
        {
          u: '@coffeelover_ng',
          p: 'Instagram',
          t: 'When will the new content launch?',
          tm: '12m',
        },
        {
          u: '@jakethebaker',
          p: 'X',
          t: 'Your latest post was fire! 🔥',
          tm: '1h',
        },
        {
          u: 'Adaeze Okonkwo',
          p: 'LinkedIn',
          t: 'Would love to discuss a partnership.',
          tm: '3h',
        },
        {
          u: '@morning_fan',
          p: 'Instagram',
          t: 'Do you ship to Abuja?',
          tm: '5h',
        },
...
const PerformancePage = ({
  ...
  <SubPage
    title="Performance Memos"
    icon="chart"
    desc="Analysis of your brand performance"
    onJane={onJane}
  >
    <div
      style={{
    ...
    >
      {[
        ...
        >
          <div style={{ fontSize: 11.5, color: '#999', marginBottom: 5 }}>
            {s.l}
          </div>
    ...
    >
      ...
      >
        <JaneAvatar size={22} />
        <span style={{ fontSize: 13, fontWeight: 700, color: '#C2185B' }}>
          URI Agent Memo
        </span>
      </div>
      <p style={{ fontSize: 13, color: '#555', lineHeight: 1.7, margin: 0 }}>
        Connect your social accounts in{' '}
        <strong>Settings → Social Accounts</strong> to start tracking real
        performance data for <strong>{brandName}</strong>. Once connected, I'll
        surface insights and recommendations here weekly.
      </p>
    </div>
  </SubPage>
);
...
const IntelPage = ({ onJane }: { onJane: () => void }) => (
  <SubPage
    title="Market Intel"
    icon="globe"
    desc="What people say about your brand across the web"
    onJane={onJane}
  >
    <div
      style={{
    ...
    >
      ...
      <p
        style={{ fontSize: 13, color: '#888', marginTop: 12, lineHeight: 1.6 }}
      >
        Market intelligence will appear here once your brand profile is fully
        set up and social accounts are connected.
      </p>
    </div>
  </SubPage>
);
...
const PlaybookPage = ({
  ...
  <SubPage
    ...
    icon="book"
    desc="Everything URI Agent knows about your brand"
    onJane={onJane}
  >
    {[
      {
      ...
      {
        t: 'Voice',
        items: [profile?.derived_voice].filter(Boolean) as string[],
      },
      {
        t: 'Content Pillars',
        items: (profile?.content_pillars ?? []) as string[],
      },
      {
        t: 'Guardrails',
        items: [
          profile?.guardrails?.avoid_topics
            ? `Avoid: ${profile.guardrails.avoid_topics}`
            : null,
          profile?.guardrails?.emoji_usage
            ? `Emoji: ${profile.guardrails.emoji_usage}`
            : null,
        ].filter(Boolean) as string[],
      },
    ].map((s) => (
        ...
        {(s.items.length > 0 ? s.items : ['—']).map((item, i) => (
          <div
            ...
            style={{
              ...
              padding: '4px 0',
              borderBottom:
                i < s.items.length - 1 ? '1px solid #f5f4f0' : 'none',
            }}
          >
            {item}
          </div>
...
const SettingsPage = ({
  ...
  <SubPage
    title="Settings & Plan"
    icon="settings"
    desc="Manage your account, billing, and team"
    onJane={onJane}
  >
    <div
      style={{
    ...
    >
      <h3 style={{ fontSize: 13.5, fontWeight: 700, marginBottom: 10 }}>
        Brand
      </h3>
      <div style={{ fontSize: 12.5, color: '#555', padding: '7px 0' }}>
        {brandName || 'No brand name set'}
      </div>
    </div>
  </SubPage>
);
...
export default function WorkspaceDashboard() {
        ...
        content: (
          <div>
            <p style={{ margin: 0 }}>
              ...
              Here's your briefing for <strong>{name}</strong>:
            </p>
            <ul
              style={{
                margin: '6px 0 0',
            ...
            >
              ...
              <li>
                Type a topic below and I'll draft content across your platforms
              </li>
              <li>
                Connect social accounts in <strong>Settings</strong> to unlock
                analytics
              </li>
            </ul>
          </div>
  ...
  useEffect(() => {
    const iv = setInterval(
      () => setSIdx((i) => (i + 1) % STATUS_MSGS.length),
      5000,
    );
  ...
  const sendMsg = async () => {
    ...
    setFeed((f) => [
      ...f,
      { id: 'u' + Date.now(), type: 'user', time: now, content: txt },
    ]);
    setTyping(true);
    ...
    try {
          ...
          setFeed((f) => [
            ...f,
            {
              ...
              content: (
                <div>
                  <p
                    style={{ margin: '0 0 10px', fontSize: 13, color: '#444' }}
                  >
                    Generated {drafts.length} draft
                    {drafts.length > 1 ? 's' : ''}. Check{' '}
      ...
      let reply: ReactNode = (
        <p style={{ margin: 0 }}>
          Got it! To generate posts, just tell me what topic or campaign you
          want content for.
        </p>
      );
      if (
        ...
        reply = (
          <p style={{ margin: 0 }}>
            Check the <strong>Posting Schedule</strong> tab to see your full
            queue, drafts, and scheduled posts — all in one place with the
            weekly calendar.
          </p>
        );
        ...
        reply = (
          <p style={{ margin: 0 }}>
            Connect your social accounts in{' '}
            <strong>Settings → Social Accounts</strong> and I'll pull real
      ...
      setFeed((f) => [
        ...f,
        { id: 'j' + Date.now(), type: 'jane', time: now, content: reply },
      ]);
    } catch {
      setTyping(false);
      setFeed((f) => [
        ...
        >
          ...
          >
            ...
            >
              ...
              <div>
                <div
                  style={{ fontSize: 12.5, fontWeight: 700, color: '#f3d0df' }}
                >
                  URI Agent
                </div>
            ...
            {NAV.map((n) => (
              <button
                ...
                style={{
                  ...
                  padding: '9px 18px',
                  background:
                    nav === n.id ? 'rgba(194,24,91,.1)' : 'transparent',
                  border: 'none',
                  borderLeft:
                    nav === n.id
                      ? '2.5px solid #E91E63'
              ...
              >
                <I
                  n={n.icon}
                  s={15}
                  c={nav === n.id ? '#E91E63' : 'rgba(255,255,255,.22)'}
                />
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
              ...
              <span style={{ fontSize: 12.5, color: '#666' }}>
                <strong style={{ color: '#C2185B' }}>URI Agent</strong> is
                active:{' '}
                <span
                  key={sIdx}
                  style={{
                    animation: 'wStatusFade 5s linear',
            ...
            {nav === 'workspace' ? (
              ...
              >
                ...
                >
                  ...
                  >
                    <div
                      style={{ flex: 1, height: 1, background: '#edecea' }}
                    />
                    <span
                      style={{
                        fontSize: 11,
                ...
                >
                  ...
                  <div style={{ textAlign: 'center', marginTop: 5 }}>
                    <span style={{ fontSize: 10, color: '#ccc' }}>
                      URI Agent can make mistakes. Always review before
                      publishing.
                    </span>
                  </div>
                </div>
