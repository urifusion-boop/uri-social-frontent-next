  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
        <p style={{ margin: 0, fontSize: 13.5, fontWeight: 700, color: '#222', display: 'flex', alignItems: 'center', gap: 6 }}>
          <CalendarPlus size={15} strokeWidth={2} />
          Keep it running
        </p>
        <button
          type="button"
          onClick={() => { if (done) onExtended?.(); onClose(); }}
      ...
      {done ? (
        <div style={{ background: '#f6fbf6', border: '1px solid #cde9cd', borderRadius: 8, padding: '10px 12px' }} data-testid="keep-running-done">
          ...
          <p style={{ margin: 0, fontSize: 12, color: '#33691e' }}>
            {naira(done.charged)} taken from your wallet. Same campaign and same ad — it keeps
            everything Meta has learned. It runs until that date unless you pause it.
          </p>
        </div>
      ) : (
        <>
          <p style={{ margin: '0 0 10px', fontSize: 12, color: '#666' }}>
            Carries on the same campaign and the same ad, so it keeps everything Meta has
            learned about who responds. Starting a new campaign instead begins that from scratch.
          </p>
          ...
          {quote && !quote.affordable && (
            <div style={{ marginTop: 10, background: '#fff8ec', border: '1px solid #f0e0c0', borderRadius: 8, padding: '8px 10px' }}>
          ...
          {error && (
            <p data-testid="extend-error" style={{ margin: '10px 0 0', fontSize: 12, color: '#b3261e' }}>{error}</p>
          )}
