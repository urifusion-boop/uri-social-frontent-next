...
export default function CustomGuidesGallery({
  ...
  const loadGuides = async () => {
    try {
      ...
      if (response.status && response.responseData) {
        setGuides(response.responseData.guides);
      }
    } catch (error: unknown) {
  ...
  const handleRematch = async (guideId: string) => {
    try {
      ...
      const response = await CustomVisualGuideService.rematchFonts(guideId);
      if (!response.status || !response.responseData) {
        throw new Error(response.responseMessage || 'Failed to rematch');
      }
          ...
          g.id === guideId
            ? {
                ...
                typography_match: {
                  ...
                  match_confidence: result.match_confidence as 'high' | 'medium' | 'low' | undefined,
                },
              }
            : g
        )
