      ...
      <Box sx={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: '12px', p: 3, mb: 2 }}>
        ...
        <Box display="flex" alignItems="center" justifyContent="space-between" mb={0.5}>
          ...
          <Box display="flex" alignItems="center" gap={1}>
            ...
            <Tooltip
              title="When on, URI Agent will automatically generate draft posts on your chosen schedule — no manual prompting needed"
              arrow
              placement="left"
              enterTouchDelay={0}
        ...
        <Typography fontSize="13px" color="#6B7280" mb={2.5}>
          Analyses your top-performing posts and auto-generates platform drafts for your approval, personalised using
          your brand profile.
        </Typography>
        ...
        <Box display="flex" alignItems="center" gap={1} mb={0.5}>
          ...
          <Tooltip
            title="Daily: new drafts every morning. Weekly: a batch of drafts once per week"
            arrow
            enterTouchDelay={0}
            leaveTouchDelay={3000}
          >
        ...
        >
          <Box>
            <Typography fontSize="13px" color="#374151">
              ...
              {settings?.last_run_at
                ? `${formatDate(settings.last_run_at)}${
                    settings.last_run_draft_count != null
                      ? ` · ${settings.last_run_draft_count} draft${settings.last_run_draft_count !== 1 ? 's' : ''} created`
                      : ''
                  }`
                : 'Never'}
            </Typography>
            <Typography fontSize="13px" color="#374151" mt={0.5}>
              ...
              {settings?.next_run_at && settings?.enabled ? formatDate(settings.next_run_at) : '—'}
            </Typography>
          </Box>

          <Tooltip
            title="Run auto-generation immediately — URI Agent will create a fresh set of drafts right now, without waiting for the next scheduled run"
            arrow
            placement="left"
            enterTouchDelay={0}
            leaveTouchDelay={3000}
          >
            <span>
              <Button
                ...
                disabled={triggering}
                startIcon={
                  triggering ? <CircularProgress size={14} sx={{ color: '#fff' }} /> : <MdAutoAwesome size={16} />
                }
                sx={{
                  textTransform: 'none',
                  fontWeight: 600,
...
const AnalyticsContextCard = ({ ctx }: AnalyticsContextCardProps) => {
  ...
  return (
    ...
    >
      <Box mt={0.2}>
        {connected ? <MdVerified size={20} color="#16A34A" /> : <MdLinkOff size={20} color="#9CA3AF" />}
      </Box>
      <Box flex={1}>
        <Box display="flex" alignItems="center" gap={1} mb={0.5}>
        ...
        {connected ? (
            ...
            <Box display="flex" flexWrap="wrap" gap={0.75}>
              {(ctx?.connected_platforms ?? []).map((p) => (
                <Chip
                  ...
                  size="small"
                  sx={{ background: '#DCFCE7', color: '#15803D', fontWeight: 600, fontSize: '11px', height: 22 }}
                />
              ))}
              {(ctx?.industries_detected ?? []).map((ind) => (
                <Chip
                  ...
                  label={ind}
                  size="small"
                  sx={{ background: '#EDE9FE', color: '#6D28D9', fontWeight: 600, fontSize: '11px', height: 22 }}
                />
              ))}
          ...
          <Typography fontSize="12px" color="#9CA3AF">
            Go to <strong>Account Tracking</strong>, open any account and click Analyse. The AI media report will
            automatically sync here to power richer content generation.
          </Typography>
        )}
      </Box>
    </Box>
