...
export default function CustomGuideUploadModal({
  ...
  const processImageUpload = async (upload: UploadingImage) => {
    ...
    try {
      ...
      const imageUrl = await CustomVisualGuideService.uploadImageFile(upload.file);
      ...
      const guide = await CustomVisualGuideService.uploadReferenceImage(
        imageUrl,
        name,
        brandId
      );
      ...
      ToastService.showToast(error.message || 'Failed to process image', ToastTypeEnum.Error);
    }
  };
