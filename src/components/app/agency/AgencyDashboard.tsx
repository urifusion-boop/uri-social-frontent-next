...
import { FiPlus, FiUsers, FiGrid, FiBarChart2, FiCopy, FiX, FiEdit2, FiCheck } from 'react-icons/fi';
    ...
    <div style={{ padding: '60px 24px', maxWidth: 520, margin: '0 auto', textAlign: 'center' }}>
      ...
      <input
        ...
        style={{
          ...
          padding: '12px 14px',
          borderRadius: 10,
          border: `2px solid ${name ? URI_PINK : '#e5e7eb'}`,
          fontSize: 15,
          outline: 'none',
          marginBottom: 16,
      ...
      <button
        ...
        onClick={async () => {
          ...
          try {
            ...
            if (res.status && res.responseData) {
              ToastService.showToast('Agency created', ToastTypeEnum.Success);
              onCreated(res.responseData);
            } else ToastService.showToast(res.responseMessage || 'Failed', ToastTypeEnum.Error);
          } catch {
            ToastService.showToast('Failed to create agency', ToastTypeEnum.Error);
          } finally {
            setBusy(false);
          }
        }}
        style={{
          ...
          borderRadius: 10,
          border: 'none',
          background: !name.trim() || busy ? '#e5e7eb' : `linear-gradient(135deg, ${URI_PINK}, #E94396)`,
          color: !name.trim() || busy ? '#9ca3af' : 'white',
          fontSize: 15,
          fontWeight: 700,
...
function EditableAgencyName({ agency, onChange }: { agency: Agency; onChange: (a: Agency) => void }) {
  ...
  const save = async () => {
    ...
    if (!trimmed || trimmed === agency.name) {
      setEditing(false);
      setName(agency.name);
      return;
    }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <button style={primary} onClick={() => setShowAdd(true)}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {brands.map((b) => (
            <div
              ...
              style={{
                background: '#fff',
                border: `1px solid ${activeBrand === b.brand_id ? URI_PINK : '#e5e7eb'}`,
                borderRadius: 12,
                padding: 18,
            ...
            >
              ...
              <button
                style={{ ...primary, width: '100%', justifyContent: 'center' }}
  ...
  const submit = async () => {
    ...
    try {
      const res = templateId
        ? await AgencyService.duplicateBrand({
            template_brand_id: templateId,
            name: name.trim(),
            industry: industry || undefined,
          })
        : await AgencyService.addBrand({ name: name.trim(), industry: industry || undefined });
      if (res.status) {
        ToastService.showToast('Brand added', ToastTypeEnum.Success);
        onAdded();
      } else ToastService.showToast(res.responseMessage || 'Failed', ToastTypeEnum.Error);
    } catch {
      ToastService.showToast('Failed to add brand', ToastTypeEnum.Error);
    } finally {
      setBusy(false);
    }
  };
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div>
                  ...
                  <span
                    style={{
                      ...
                      fontSize: 12,
                      fontWeight: 700,
                      color: m.role === 'admin' ? URI_PINK : '#6b7280',
                ...
                {m.role !== 'admin' && (
                  <button
                    style={ghost}
                    onClick={async () => {
                      await AgencyService.removeMember(m.agency_member_id);
                      load();
                    }}
                  >
                    Remove
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {brands.map((b) => {
                    ...
                    return (
                      <button
                        key={b.brand_id}
                        onClick={() => toggleBrand(m, b.brand_id, assigned)}
                        style={{
                          padding: '4px 10px',
                          borderRadius: 16,
                          fontSize: 12,
                          cursor: 'pointer',
                          border: `1px solid ${assigned ? URI_PINK : '#e5e7eb'}`,
                          background: assigned ? `${URI_PINK}10` : '#fff',
                          color: assigned ? URI_PINK : '#6b7280',
    ...
    <Modal title="Invite member" onClose={onClose}>
      ...
      <button
        ...
        onClick={async () => {
          ...
          if (res.status) {
            ToastService.showToast('Member invited', ToastTypeEnum.Success);
            onInvited();
...
function ReportsSection() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await AgencyService.getPortfolioReport();
        setReport(r.status ? (r.responseData ?? null) : null);
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: '8px 16px',
        borderRadius: 8,
        border: `2px solid ${active ? URI_PINK : '#e5e7eb'}`,
        background: active ? `${URI_PINK}10` : '#fff',
        color: active ? URI_PINK : '#6b7280',
        fontSize: 14,
        fontWeight: 600,
        cursor: 'pointer',
      }}
  ...
  return (
    <span
      style={{
        padding: '3px 8px',
        borderRadius: 6,
        background: highlight ? `${URI_PINK}10` : '#f3f4f6',
        color: highlight ? URI_PINK : '#6b7280',
        fontWeight: 600,
      }}
    >
      {value} {label}
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#fff', borderRadius: 16, padding: 24, width: 'min(440px, 92vw)' }}
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
...
const primary: React.CSSProperties = {
  ...
  borderRadius: 8,
  border: 'none',
  background: `linear-gradient(135deg, ${URI_PINK}, #E94396)`,
  color: 'white',
  fontSize: 14,
  fontWeight: 700,
  cursor: 'pointer',
