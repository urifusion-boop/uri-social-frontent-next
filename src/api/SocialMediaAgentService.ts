...
export interface CalendarDayItem {
  ...
  final_score?: number;
  reason?: string;
  // Coherent per-idea fields — all generated by the same AI call as title/description,
  // so they're guaranteed to relate to this specific idea rather than a generic template.
  hook?: string; // exact opening line for the post
  key_points?: string[]; // concrete points that deliver on the title's promise
  caption_direction?: string; // specific guidance for writing this exact caption
  keywords?: string[]; // 2-4 real keywords/hashtag-worthy terms for this idea
  cta?: string; // call-to-action specific to this idea
...
// ── Content Calendar V2 — a distinct, isolated 30-day engine now used in
// production instead of the v1 types above (never extended from them) so the
// two systems' shapes can never silently couple. See ContentCalendarV2Tab.tsx
// for the isolation note. ──
