    ...
    <div className="min-h-screen">
      <section className="pt-28 pb-16" style={{ backgroundColor: 'hsl(var(--background))' }}>
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1
            className="comic-headline text-4xl sm:text-5xl lg:text-6xl font-black mb-5"
          ...
          <p className="font-bold text-lg max-w-2xl mx-auto" style={{ color: 'hsl(var(--muted-foreground))' }}>
            For developers who want to integrate Jane's capabilities into their own products. Available on the Executive
            plan.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <ScrollReveal>
            <div className="comic-panel p-6" style={{ backgroundColor: 'hsl(var(--background))' }}>
              <h2 className="text-lg font-black uppercase mb-4" style={{ color: 'hsl(var(--foreground))' }}>
                Getting Started
              </h2>
              <ul className="space-y-2 text-sm font-bold" style={{ color: 'hsl(var(--foreground) / 0.7)' }}>
                <li>
                  <strong style={{ color: 'hsl(var(--foreground))' }}>Base URL:</strong> https://api.urisocial.com/v1
                </li>
                <li>
                  <strong style={{ color: 'hsl(var(--foreground))' }}>Auth:</strong> API keys (generated in HR & Payroll
                  → API Access)
                </li>
                <li>
                  <strong style={{ color: 'hsl(var(--foreground))' }}>Rate limits:</strong> 100 requests/minute on
                  Executive plan
                </li>
                <li>
                  <strong style={{ color: 'hsl(var(--foreground))' }}>Response format:</strong> JSON
                </li>
              </ul>
            </div>
          </ScrollReveal>
          ...
          <ScrollReveal>
            <div className="comic-panel p-6" style={{ backgroundColor: 'hsl(var(--foreground))' }}>
              <pre
                className="text-sm font-mono overflow-x-auto whitespace-pre"
  ...
  "format": "carousel"
                ...
                {endpoints.map((e, i) => (
                  <div key={i} className="flex flex-wrap items-start gap-x-3 gap-y-1 text-sm">
                    <span
                      ...
                      style={{
                        backgroundColor:
                          e.method === 'GET' ? 'hsl(var(--uri-blue) / 0.1)' : 'hsl(var(--uri-green) / 0.1)',
                        color: e.method === 'GET' ? 'hsl(var(--uri-blue))' : 'hsl(var(--uri-green))',
                      }}
                    >
                      {e.method}
                    </span>
                    <code className="font-mono font-bold" style={{ color: 'hsl(var(--foreground))' }}>
                      {e.path}
                    </code>
                    <span className="font-bold" style={{ color: 'hsl(var(--foreground) / 0.5)' }}>
                      — {e.description}
                    </span>
                  </div>
                ))}
          ...
          <ScrollReveal>
            <div
              ...
              style={{ backgroundColor: 'hsl(var(--primary) / 0.05)', borderColor: 'hsl(var(--primary))' }}
            >
              <h3 className="text-base font-black uppercase mb-2" style={{ color: 'hsl(var(--foreground))' }}>
                Full documentation coming soon
              </h3>
              <p className="text-sm font-bold mb-4" style={{ color: 'hsl(var(--foreground) / 0.7)' }}>
                Want early access?
              </p>
              <button
              ...
              <p className="text-xs font-bold mt-4" style={{ color: 'hsl(var(--muted-foreground))' }}>
